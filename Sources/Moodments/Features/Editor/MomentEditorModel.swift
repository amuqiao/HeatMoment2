...
/// `docs/current/implementation-truth.md` §4.4、`docs/product-mental-model.md` §3.1）。
///
...
///
/// 限额判定只消费 `QuotaService` 结果（见 `docs/current/implementation-truth.md` §6），本类型不
/// 重复定义限额数值；照片额度校验直接用 `draftPhotos.count`（草稿即完整当前状态，编辑态
/// 载入的既有照片也计入其中，无需另外查仓库）。
///
/// **Pro 放行判定**（见阶段7计划决策1、docs/current/implementation-truth.md §11.4）：照片额度闸门
...
final class MomentEditorModel {
    ...
    /// 打开时的字段快照，供 `isDirty` 比较（见 docs/product-mental-model.md §3.1：取消若脏需二次确认）。
    ...
    /// （见阶段 4 计划 §8 延后项、`docs/current/local-data-architecture.md` §5 缩略图缓存清理）。
    ...
    /// 标题/正文/照片至少一项非空才可保存（见 docs/product-mental-model.md §3.1）。
    var canSave: Bool {
    ...
    // MARK: - 标签（见 docs/current/implementation-truth.md §4.6/§4.7）
    ...
    // MARK: - 照片（见 docs/current/implementation-truth.md §4.4，压缩管线见 `ImageCompressor`）

    /// 追加照片前的额度校验：先现场重查 Pro 权威判定（决策1），UI 只消费该结果，不自行比较
    /// 数值（见 docs/current/implementation-truth.md §6）。
    ...
    /// （决策1、docs/current/implementation-truth.md §6 不变），本方法只是把「查 Pro」这一步 IO 从循环体内提到
