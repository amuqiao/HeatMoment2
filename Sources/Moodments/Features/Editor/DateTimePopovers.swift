...
/// （日期 / 时:分），合成时保留另一部分不变（见 `docs/design/04-screen-specs.md` §4.8）。
enum OccurredAtComposer {
    /// 用 `date` 的年/月/日 + `timeSource` 的时/分/秒合成新的 `Date`。
    static func mergingDate(_ date: Date, timeFrom timeSource: Date, calendar: Calendar = .current) -> Date {
    ...
    static func mergingTime(_ time: Date, dateFrom dateSource: Date, calendar: Calendar = .current) -> Date {
        mergingDate(dateSource, timeFrom: time, calendar: calendar)
    }
}

/// 发生时间·日期就近浮窗（见 04-screen-specs.md §4.8）：graphical 日历，支持选择任意
/// 过去/未来日期以支持补记；选择后立即用 `Calendar` 合成回填 `occurredAt`（保留原时:分），
/// 无需额外确认按钮。
...
/// `Calendar` 合成回填 `occurredAt`（保留原年月日），无需额外确认按钮。
