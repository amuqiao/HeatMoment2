...
/// 类型系统本身就杜绝了热力图定位污染数据集的可能；`scrollTargetID(for:in:)` 是纯函数，
/// 只在调用方已经算好的「当前可见（已筛选）集」里挑一个 id，既不返回、也不修改数据集本身。
/// 两个函数彼此不引用、不共享任何中间状态，`TimelineListView` 里也分别独立驱动
/// （`@Query` = f(filter)；`.onChange(of: heatmapFocusDate)` = f(focusDate, entries)）。
enum TimelineQuery {
