...
/// `LocalAuthentication` 封装（见 `docs/design/10-security-privacy.md` §10.1.1）：`LAContext`
/// 非 `Sendable`，实例只在本服务内部临时创建/使用，不跨隔离域传递（见
/// `docs/design/08-architecture.md` §5）。`.deviceOwnerAuthentication` policy 本身即「优先生物
/// 识别、不可用时兜底系统密码」（10 §10.1.1「无生物识别兜底」），不需要本类型自行实现降级分支。
