...
/// 购买结果，供 `ProPaywallView` 决定如何反馈（见 `docs/design/11-monetization.md` §11.2）。
...
/// StoreKit 2 订阅/买断集成（见 `docs/design/11-monetization.md`）：商品加载、购买、恢复购买、
/// 核销码入口、entitlement 核对与本地缓存刷新。
///
/// **并发边界**（见 `docs/design/08-architecture.md` §5、阶段7计划必守约束）：本类型 `@MainActor`
...
///
/// **状态一致性**（11 §11.4）：`isPro` 只是「当次查询完成后缓存下来的快照」，供 UI 快速展示
...
final class SubscriptionService {
    ...
    init(cacheStore: SubscriptionStateCacheStore = SubscriptionStateCacheStore()) {
        ...
        // 权威判定仍会在 `refreshEntitlements()` 中现场重查并覆盖（见 11 §11.4）。
    ...
    /// 启动时开长任务监听 `Transaction.updates`（见 11 §11.1）：实时接收续订/退款/取消/兑换码
    ...
    /// 权威判定（11 §11.4）：现场重查 `Transaction.currentEntitlements`，命中「月订阅未过期」
    /// 或「终身买断已购」任一即 Pro（OR 关系，见 11 §11.1）。副作用：刷新本地缓存与 `isPro`
    ...
    /// 启动 / 回前台调用（见 11 §11.1）：语义上等价 `currentEntitlementIsPro()`，只是标注调用
    /// 时机，供 `MoodmentsApp` 复用。
    func refreshEntitlements() async {
        _ = await currentEntitlementIsPro()
    }

    /// 加载商品信息（`ProPaywallView` 展示 `Product.displayPrice` 动态渲染用，见 11 §11.1）。
    ...
    /// 购买（见 11 §11.2）：`.success` 校验签名后 `finish()` + 现场重查刷新；`.userCancelled`/
    ...
    /// 恢复购买（见 11 §11.2）：`AppStore.sync()` 后重新核对，返回是否命中任一有效 Pro 授权，
    /// 供 UI 即时反馈成功/未找到记录。
    ...
    /// 是否可展示核销码入口（见 11 §11.2：仅 iOS 平台且 `canMakePayments` 为真时展示）。
    static var canMakePayments: Bool { AppStore.canMakePayments }

    /// 唤起系统兑换码弹层（见 11 §11.2）：兑换成功依赖 `Transaction.updates` 被动接收新授权，
    /// 本方法不轮询。
