...
/// 订阅状态本地缓存（见 `docs/design/07-data-persistence.md` §7）：**不接入 CloudKit**——
...
/// （11 §11.4，见 `SubscriptionService.currentEntitlementIsPro()`）。
struct SubscriptionStateCache: Codable, Sendable, Equatable {
    /// = `subscriptionActive || lifetimeUnlocked`（OR 结果，见 11 §11.1）。
    var isPro: Bool
    /// 月订阅当前有效。
...
/// `SubscriptionStateCache` 的 `UserDefaults` 持久化（Codable JSON blob，见 07 §7）。
///
...
/// （11 §11.4），坏缓存的正确恢复路径就是「当作没有缓存、以下一次现场重查结果为准」，
/// 不是需要用户感知的错误。
struct SubscriptionStateCacheStore {
    private static let key = "com.moodments.subscriptionStateCache"
