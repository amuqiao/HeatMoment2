...
///
/// `rawValue` 一旦发布上线即视为**持久化契约**（见 `docs/design/06-domain-model.md` §1）：
/// - 禁止重排、禁止复用旧值；
...
enum Mood: Int, CaseIterable, Codable, Identifiable, Sendable {
    ...
    /// emoji 字面量，与情绪选择器 / 时间轴节点 / 心情统计条完全一致（见 06-domain-model.md §1.4）。
    var emoji: String {
        switch self {
        case .normal: "😀"
        case .happy: "😄"
    ...
    /// i18n key，如 `"mood.normal.name"`；对应 zh-Hans / en 两套本地化文案（见 06-domain-model.md §1.1）。
