...
/// 时间轴心情节点（见 docs/current/implementation-truth.md §5.5）：直径约 12–14pt 的纯色圆点，
/// 填充色为**该条时刻情绪对应的心情色**（见 `MoodPalette`），不是固定色、不是统一色点
/// （依 `product-mental-model.md` 公理1「心情色一致性」）。
///
/// 情绪辨识不单靠节点颜色——气泡内容/情绪选择器/统计页均有 emoji + 名称冗余（见 docs/current/implementation-truth.md §5.10.3），
