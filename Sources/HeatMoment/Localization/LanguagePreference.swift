...
/// 「跟随系统」三选一，`AppStorage` 持久化，驱动 `HeatMomentApp` 注入 `.environment(\.locale, ...)`。
///
...
enum LanguagePreference: String, CaseIterable, Identifiable, Sendable {
    ...
    /// `AppStorage`/`UserDefaults` 持久化 key（`LanguageSettingsView` 与 `HeatMomentApp` 共享
    /// 同一 key，读写同一份 `UserDefaults.standard`，天然保持一致，无需额外的变更通知管线）。
    static let storageKey = "com.heatmoment.languagePreference"
    ...
    /// `HeatMomentApp` 的 `@AppStorage(storageKey)` 读写同一 key），缺省（从未设置）时为
    /// `.zhHans`（见类型头部说明）。
