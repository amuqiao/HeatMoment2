...
- 缩略图继续作为可丢弃缓存。
- 继续实现 recovery/export/sync pin 表和完整 asset GC 的执行入口。
...
验收：加图、删图、恢复、彻底删除、恢复点保留和导出期间都不会误删资产；baseline import 已能证明 asset metadata 与 content-addressed 文件字节一致，reachability audit 已能发现 missing/corrupt/orphan/unlinked/negative pin 状态，GC dry-run 与 DB orphan finalizer 已覆盖普通 DB orphan 记录，但该阶段还不能关闭，直到 pin 表和完整 GC 执行入口完成。
