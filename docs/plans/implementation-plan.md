...
| 外观主题 | `173`、`174` | 外观页应通过预览表达模式、主色、背景和图片展示的真实差异。 | P2a 已补首页主场景背景消费闭环；外观页 A 方案已接入分组缩略卡和 token 消费边界，as-built 见 current。 | P2 后续只做亮色细节视觉验收和页面级漂移精修。 |
...
| P2 | 外观页模式总览 / 亮色适配 | 外观设置已具备模式总览、主色、背景纹理和图片展示四组可感知控件；背景设置消费闭环已在 P2a 关闭。剩余是亮色截图审计和细节精修。 |
...
6. 外观页审计（P2）
   - 参考 Flutter 外观详情页的设计理念：外观轴分离为显示模式、主色、背景；照片显示作为内容行为偏好单独表达，不和主题轴混为一类。
   - 参考最初设计稿 `173` / `174` 的预览驱动语义：模式、纹理和图片展示必须让用户看到真实差异；列表文字和勾选只能作为过渡态，不作为最终验收态。2026-07-08 已按 A 方案改为设计稿式分组结构。
   - 已为模式、主色、背景、图片展示定义可感知预览。模式预览承担当前主题总览，体现明暗背景、导航/文字层、当前背景纹理/自定义图片和 FAB/当前主色；主色预览用响应式 swatch 和选中态表达；背景预览只展示网格/点阵/无纹理/自定义图片的纯背景差异，不重复展示 FAB；图片展示预览只表达滚动/轮播行为，不改变颜色语义。
...
- P2b 已落地的 token 消费边界和外观页模式总览/背景样本事实记录在 `docs/current/implementation-truth.md`。
...
- 外观页模式总览和背景样本已接入，但仍需在后续视觉验收中确认不同屏宽、亮色模式和自定义背景图片下的排版/对比。
...
- 外观页模式总览、背景样本和真实首页/设置/热力图组件消费同一套 token；切换主色后，同一张模式总览卡会随当前主色同步变化，暗/亮两张模式卡也保持稳定可见差异。当前已有定向 UI 测试覆盖这两条边界。
...
- 验证层：
  ...
  - P2 触达范围内不得新增散落硬编码颜色；新增或调整颜色先进入 SwiftUI token，再由页面消费。
