...
| 编辑与就地选择 | `162`、`163`、`165`、`166`、`167`、`168` | 编辑器是任务卡片；情绪、标签、日期、时间是字段级就地选择；照片区有“添加前 CTA / 添加后资产管理”两态。 | 时间 popover 即时回写仍缺窄测试；`164` 暗示标签选择内可 `+添加`，且公理层仍写有“编辑时临时创建”，但 current/design 已收束到设置页标签管理。 | P1 补日期/时间回写测试；标签创建归属先作为四层文档冲突处理，统一公理/design/current 后再实现。 |
| 统计回看 | `170`、`171`、`178` | 统计页是设置流中的年度回看，不是一级仪表盘；热力图和统计条形共用心情色语义。 | 亮色统计页也需验证心情色和文本可读性。 | 并入 P2 亮色主题验收。 |
| 设置服务中枢 | `169`、`172` | 设置是支撑能力中枢，不是第二个主场景；标签管理是轻量管理，不是复杂 taxonomy 系统。 | 当前设置根 sheet 有显式“关闭”按钮；是否保留仍需按页面感和系统 sheet 关闭手势裁决。 | P1b 路径审计时裁决；未裁决前保持 current 事实。 |
...
| 编辑器时间选择测试 | 日期 popover 已有打开/收起覆盖，时间 popover 与 `OccurredAtComposer` 即时回写仍缺窄测试。 | 先补测试锁住“只改日期 / 只改时间”的语义，再考虑视觉微调。 |
| 设计稿与契约偏离 | 最初设计稿暗示空月份可定位；设计层已裁决为只定位真实记录月份，current 已落地。 | 不默认按设计稿回改；若要改变，先在 `13-open-questions.md` 重新打开 #21。 |
| 标签创建归属冲突 | 最初设计稿 `164` 暗示编辑器内可新建标签；公理层仍写“编辑时临时创建”，但 design/current 已把标签创建归设置页。 | 这是四层文档冲突，不是单纯实现缺口；进入实现前先裁决并同步公理层、设计层、current 或计划。 |
...
4. Sheet 层级审计（P1b）
   ...
   - 统一标签创建归属：选项 A 为遵循公理层现有表述，在编辑器内提供临时创建并回填当前 Moment；选项 B 为修正公理层表述，保持 current/design 的设置页集中创建。裁决后同步公理层、设计层、current 或计划，避免四层互相矛盾。

5. 编辑页局部选择审计（P1b）
   - 保留日期/时间 popover 作为正向范式。
   - 只检查它与脏数据关闭、保存后回上下文、VoiceOver 焦点的衔接，不重做基础 picker。
   - 补日期/时间即时回写窄测试，覆盖“只改日期”和“只改时间”不互相污染。
...
- 文档层：
  ...
  - 筛选呈现已裁决为首页就地半屏 sheet；标签创建归属完成裁决并同步后，公理层、设计层、current 层和计划层之间不再保留互相矛盾的描述。
