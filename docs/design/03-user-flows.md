...
- 左滑卡片时，日期、心情节点、moment 气泡作为一个记录整体左移，露出删除动作；点击后 Moment 立即软删除进入垃圾箱，**不弹二次确认弹窗**（因为垃圾箱本身即恢复安全网）`[观测确认 + 设计决策]`。交互参考 iOS 系统列表的成熟 swipe action：轻扫露出删除按钮，继续滑动时删除按钮拉长；不为复刻 Flutter 自定义滑动系统而重写基础手势。
...
| 操作对象 | 选择年份 → 选择某月/某有记录日期；月份入口隐藏在热力图月份标签/月份区域中 | 选择**多个标签（彼此 AND 交集）+ 至多一个心情（单选）**；标签与心情之间也 AND（见《04-screen-specs.md》4.2 筛选组合逻辑） |
| 对时间轴的影响 | **不改变可见数据集合**，只是把时间轴滚动定位到对应时间段，并更新时间上下文标记和定位高亮 `[真机确认 + 观测确认]` | **改变可见数据集合**，只展示命中条件的 Moment，并在首页顶部显示条件横条（如「相关标签 #工作」+ X）`[真机确认]` |
| 选中态 | 点月时，以当前设置页主色一致的轻微透明蒙层覆盖对应月份热力图区域；点日时，显示日期级选中态 | 选中项通过 sheet 内行内勾选态和首页上下文标记表达 |
| 取消方式 | 再次点击同一锚点取消定位；点右上 X 收起热力图上下文区 `[观测确认]` | 点条件横条上的 X 单独移除该条件，或整体清除 `[真机确认]` |
...
工程实现上二者必须是两个独立状态源：`heatmapFocusDate: Date?` 或等价的时间 anchor（只影响 `ScrollViewReader` 的滚动目标与时间上下文标记）与 `activeFilter: FilterCondition?`（只影响 `@Query` 的谓词），严禁合并成同一个状态变量，否则会破坏「定位≠筛选」的产品语义 `[观测确认]`。月 anchor 命中真实记录，不生成月份伪节点；当当前 tag/mood 筛选挡住目标时间内记录时，保留筛选并展示当前条件下无记录的上下文反馈，不偷偷清除筛选。
...
- 首页删除不二次确认；垃圾箱内的彻底删除才是不可逆操作，视为用户「放弃恢复」 `[观测确认]`。首页删除手势使用系统列表 swipe action 语义，轻扫露出删除、继续滑动拉长删除按钮；恢复是相反的生命周期动作。
