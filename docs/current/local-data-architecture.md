...
阅读副本文件写在系统临时目录，进入导出页和每次新导出都会清理旧的 `HeatMoment-*` 包。导出失败会清理本次半成品包；导出成功后生成 `ExportShareTransaction` 并立即打开系统分享，分享完成或取消后结束事务并清理本次临时副本；系统分享返回错误时保留本次事务并允许重试分享；如果 App 在分享中或清理前被杀死，下次进入导出页会清理遗留临时副本。阅读副本导出不写 canonical store、不创建恢复点、不触发 iCloud 状态，也不能导回恢复。
