...
`current` 回答“现在实际怎么实现”。它不承诺未来优化，不替代计划层，也不把 Flutter 版实现细节当作 SwiftUI 事实。当前 SwiftData 路径是受控导入工具、过渡恢复测试和历史测试辅助，不是生产数据权威。
...
| 数据持久化 | M5 后，生产主 UI、用户写入、本机恢复点和 Markdown/PDF 导出路径的权威均为 GRDB canonical store。`MoodmentsApp` 启动先调用 `CanonicalBootRestoreGate` 消费 pending restore，再创建并注入 `CanonicalLibraryService` / `CanonicalLibraryRuntime` / `CanonicalRecoveryCoordinator`；`RootView` 进入主页前只执行 canonical prepare，不再读取 `modelContext` 或执行 SwiftData baseline 导入。时间轴、筛选、预览、编辑、标签管理、垃圾箱、统计/热力图、缩略图原图读取、主流程写入、导出 snapshot 和 UI 测试种子已消费 canonical 值类型与 `FileAssetStore` 原图。生产启动不再创建 SwiftData `ModelContainer`；同步状态仍是启发式展示，不代表 canonical iCloud 同步已经完成。 | `MoodmentsApp.swift`、`RootView.swift`、`CanonicalLibraryService.swift`、`LocalLibraryMutationService.swift`、`SyncStatusService.swift`、`CanonicalStore.swift`、`CanonicalLibraryRuntime.swift`、`FileAssetStore.swift`、`SwiftDataCanonicalImporter.swift`、`CanonicalLibraryRepository.swift`、`ExportService.swift` |
| 本地自动恢复点 | M3 canonical recovery cutover 已落地。设置页“备份与恢复”重新开放并使用 `CanonicalBackupRestoreService`；用户可查看最多 3 个系统自动维护的本机恢复点、进入恢复预览并准备恢复，不能删除、分享或导出恢复点。`LocalLibraryMutationService` 的 canonical backend 在普通写入成功后通过 `CanonicalRecoveryCoordinator` 创建节流稳定恢复点，高风险写入前创建 mutation safety 恢复点，安全点失败会阻断后续破坏性操作。准备恢复会 stage selected snapshot、创建 restore safety、arm pending restore 并进入阻断页；下一次启动由 `CanonicalBootRestoreGate` 在打开 runtime 前执行 replace restore。SwiftData 过渡版恢复服务仍存在于代码和测试中，但不再作为用户可触达的生产恢复入口。 | `SettingsSheetView.swift`、`LocalLibraryMutationService.swift`、`BackupRestoreService.swift`、`BackupRestoreView.swift`、`PendingLocalRestoreView.swift`、`CanonicalRecoveryCoordinator.swift`、`CanonicalRestoreExecutor.swift`、`CanonicalBootRestoreGate.swift` |
| Markdown / PDF 导出 | M4 canonical export cutover 已落地，M5 已删除 SwiftData 导出 snapshot adapter。设置页“导出”详情页可选择 Markdown 或 PDF，生产 `ExportView` 从环境中的 `CanonicalLibraryService.repository` 构造 `CanonicalExportSnapshotStore`，读取 canonical 活跃 Moment，按 `occurredAt` 倒序生成导出输入；标签按 canonical link 顺序输出，图片按 `moment_asset_link.sort_index` 从 `FileAssetStore` 读取原图 bytes。Markdown 使用 `MarkdownExportRenderer` 生成 `.md` 和相邻 `assets/` 相对图片目录，PDF 使用系统 `UIGraphicsPDFRenderer` / CoreText 本地分页并把照片嵌入 PDF。生成中禁用重复点击；失败时在详情页展示失败态和重试入口。导出仍是只读副本，不改变 canonical store，不创建恢复点，不参与 iCloud 同步；当前只支持全部活跃时刻，不支持筛选/日期范围/持久 `export_job`。 | `ExportService.swift`、`MarkdownExportRenderer.swift`、`PDFExportRenderer.swift`、`ExportView.swift` |
| Canonical 恢复点地基 | 已落地并接入生产设置页和启动路径。v4 schema 新增 `recovery_point_record` 与 `recovery_point_asset_record`，记录恢复点原因、状态、schema/app version、source library、SQLite 快照相对路径/字节数/hash、记录/标签/照片计数和 asset manifest；`CanonicalRecoveryPointStore` 在单个 GRDB 写事务内创建 catalog、写入 asset manifest、写入 `owner_kind = recoveryPoint` 的 content-hash pin，并在超过 3 个时淘汰最旧恢复点和释放对应 pin。`CanonicalRecoveryCoordinator` 提供列表、当前 counts、稳定变更节流恢复点、mutation safety 恢复点、恢复点校验、兼容性检查和 prepare restore；prepare restore 会把 restore safety catalog 写入 staged context，boot replace 时先删除 staged snapshot catalog 中缺失 snapshot 文件的恢复点、注入 restore safety，再按最多 3 个恢复点执行 retention，确保恢复后安全恢复点仍在列表中且不会展示幽灵恢复点。`CanonicalRestoreExecutor` / `CanonicalBootRestoreGate` 提供 stage / arm / boot replace / rollback 路径，critical rollback failure 会中止启动。 | `CanonicalStore.swift`、`CanonicalRecords.swift`、`CanonicalRecoveryPointStore.swift`、`CanonicalRecoveryPointSnapshotService.swift`、`CanonicalRecoveryCoordinator.swift`、`CanonicalRestoreExecutor.swift`、`CanonicalBootRestoreGate.swift`、`CanonicalMigrationSafetyGate.swift`、`CanonicalLibraryRuntime.swift` |
...
结果：通过。`MarkdownExportServiceTests` 覆盖从 canonical 活跃 Moment 快照生成 Markdown、写入相对 `assets/` 附件、保留照片字节、排除软删除记录、空库导出，以及写入失败后清理未完成导出包；`MarkdownExportRendererTests` 覆盖标题/标签 Markdown 转义、正文换行归一和图片扩展名识别；`PDFExportServiceTests` 覆盖生成可读 PDF、稳定 `.pdf` 文件名、照片计数、空库导出和坏图片失败；`PDFExportRendererTests` 覆盖长文分页、图片嵌入路径和坏图片快速失败；`MarkdownExportUITests` 覆盖设置页“导出”入口、详情页说明、生成 Markdown / PDF、成功态、系统分享入口、PDF 失败态和重试入口。Markdown 分享入口指向整个导出目录，避免只分享 `.md` 时丢失相对 `assets/`；PDF 分享入口指向单个 `.pdf` 文件。当前基础闭环只实现全部活跃时刻导出；筛选/日期范围、复杂取消流和持久 `export_job` 属于后续增强。
...
结果：全部通过。`BackupRestoreServiceTests` 覆盖 canonical 恢复点、prepare context、boot restore result 到设置页 value model 的映射；`LocalLibraryMutationServiceTests` 覆盖 canonical backend 写入后通过 `CanonicalRecoveryCoordinator` 创建稳定恢复点，以及高风险恢复写入前创建 mutation safety 恢复点；`CanonicalRecoveryCoordinatorTests` 覆盖稳定变更节流、安全点、prepare restore、arm 后 retention 失败和 selected snapshot 被淘汰后的可恢复语义；`CanonicalRestoreExecutorTests` 覆盖 armed replace、rollback、staging pin 清理、staged payload 恢复，以及 boot restore 后 pruning 旧 snapshot catalog 并保留 restore safety；`CanonicalBootRestoreGateTests` 覆盖普通启动打开 runtime 前消费 pending restore。两条 `BackupRestoreUITests` 覆盖设置页入口可进入 canonical 恢复点列表/预览、无删除入口、准备恢复后阻断继续使用，以及终止并冷启动后的 replace restore 成功提示。该阶段剩余的 Markdown / PDF 导出 canonical cutover 已由后续 M4 关闭。

2026-07-11 本轮 M4 canonical export cutover 的定向验证：
