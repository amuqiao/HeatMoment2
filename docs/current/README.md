...
| 数据持久化 | 生产 UI 路径已落地但属于过渡形态。当前用户写入仍使用 SwiftData `Moment` / `Tag` / `MomentImage`，生产启动路径会尝试 SwiftData + CloudKit 私有库容器，不可用时回退本地容器；UI 用户写入入口经 `LocalLibraryMutationService` 编排后再调用 SwiftData repository，同步状态仅由 CloudKit 是否启用、网络可达性和最近本地写入时间启发式推导。GRDB canonical local core 已有 schema、repository、`CanonicalLibraryRuntime` 装配类型、content-addressed `FileAssetStore`、SwiftData baseline 导入器、content-hash pin 表、内部 recovery point catalog store、真实 SQLite snapshot service、内部 canonical recovery coordinator、内部 canonical restore executor、内部 boot restore gate 和内部 migration safety gate；导入器已把 SwiftData 原图字节写入 asset store，并用 SQLite `asset_record` 保存 hash/MIME/尺寸/字节数 metadata。当前 App 启动不打开 canonical 持久库，canonical 尚未接入生产 UI，也不是双写路径。目标数据生命周期见计划层，不把当前 SwiftData 模型定义为最终权威。 | `ModelContainer+Config.swift`、`MoodmentsApp.swift`、`Moment.swift`、`Tag.swift`、`MomentImage.swift`、`LocalLibraryMutationService.swift`、`SyncStatusService.swift`、`CanonicalStore.swift`、`CanonicalLibraryRuntime.swift`、`FileAssetStore.swift`、`SwiftDataCanonicalImporter.swift`、`CanonicalLibraryRepository.swift`、`CanonicalRecoveryPointStore.swift`、`CanonicalRecoveryPointSnapshotService.swift`、`CanonicalRecoveryCoordinator.swift`、`CanonicalRestoreExecutor.swift`、`CanonicalBootRestoreGate.swift`、`CanonicalMigrationSafetyGate.swift` |
| 本地自动恢复点 | 已落地最小 SwiftData 过渡版。非 CloudKit 本地容器下，App 自动维护最多 3 个恢复点；普通用户写入后经 `LocalLibraryMutationService` 按稳定变更节流创建恢复点；垃圾箱恢复/彻底删除、标签删除和恢复点 replace restore 前创建操作安全点。设置页通过 `BackupRestoreServicing` 消费恢复点列表、当前摘要和准备恢复能力；当前生产适配器仍是 SwiftData `LocalBackupCoordinator`，canonical 适配器只作为后续 cutover 边界，不接生产启动。用户可恢复、不可删除恢复点；准备恢复后进入阻断页等待重启，冷启动完成后提示恢复结果。 | `LocalLibraryMutationService.swift`、`LocalBackupCoordinator.swift`、`RecoveryPointManager.swift`、`LocalBackupRestoreExecutor.swift`、`BackupRestoreService.swift`、`BackupRestoreView.swift`、`PendingLocalRestoreView.swift` |
| Markdown 导出 | 已落地 M1。设置页新增“导出”详情页，可从当前 SwiftData 活跃 Moment 快照生成 Markdown；渲染和文件写入在 UI actor 外执行；照片复制到相邻 `assets/` 并以相对链接引用；成功后显示文件名、时刻/照片数量，并通过系统分享入口分享整个导出目录。导出包位于系统临时目录，每次新导出前清理旧包。当前只支持全部活跃时刻，不支持筛选/日期范围/PDF/持久导出任务。 | `MarkdownExportService.swift`、`MarkdownExportRenderer.swift`、`MarkdownExportView.swift` |
| Canonical 恢复点地基 | 已落地内部 store/snapshot/coordinator/executor/boot gate/migration gate，不接生产 UI。v4 schema 新增 `recovery_point_record` 与 `recovery_point_asset_record`，记录恢复点原因、状态、schema/app version、source library、SQLite 快照相对路径/字节数/hash、记录/标签/照片计数和 asset manifest；`CanonicalRecoveryPointStore` 在单个 GRDB 写事务内创建 catalog、写入 asset manifest、写入 `owner_kind = recoveryPoint` 的 content-hash pin，并在超过 3 个时淘汰最旧恢复点和释放对应 pin；内部也支持暂缓 retention、显式执行 retention 和删除单个恢复点。`CanonicalRecoveryPointSnapshotService` 使用 GRDB online backup 创建真实 SQLite snapshot，先写 staging，按 snapshot 自身读取 metadata/count/asset manifest，校验 asset blob，atomic move 到 `RecoveryPoints/<id>/Library.sqlite` 后写 catalog；catalog 失败会清理 snapshot 目录，淘汰旧恢复点会同步删除旧 snapshot 目录，校验失败会把恢复点标为 `invalid`；维护入口可按当前 catalog 清理 orphan recovery point 目录和 `staging` 残留。`CanonicalRecoveryCoordinator` 是内部应用服务边界：提供列表、当前 counts、稳定变更节流恢复点、mutation safety 恢复点、恢复点校验、兼容性检查和 prepare restore；prepare restore 固定为“校验 selected -> stage selected -> 创建暂缓 retention 的 restore safety -> arm pending -> 执行 retention”，arm 前失败会清理 pending，并删除本次临时 restore safety，确保 selected 仍可重试；arm 后 retention 失败会在返回值中标记为清理未完成，但 pending restore 已准备好。`CanonicalRestoreExecutor` 已提供内部 stage / arm / boot replace / rollback 路径：stage 将 selected recovery point snapshot 复制到 `PendingRestore/payload`，把 selected record / manifest 写入 pending context，并用 `restoreStaging` content-hash pin 保护 asset；只有 armed marker 存在时启动期才替换主 canonical SQLite；恢复前会重写 staged snapshot 的 `libraryID` / `deviceID` / `syncEpoch`，成功或非 critical 失败会清理 pending，失败不替换当前 store，rollback 本身失败则作为 critical error 抛出以避免继续打开半替换 store。`CanonicalBootRestoreGate` 是薄启动闸门：调用方在打开 `CanonicalLibraryRuntime` 前可先消费 pending restore；无 pending 时不创建 canonical root 或 DB，armed pending 成功替换后再由调用方打开 runtime，unarmed pending 会清理且不替换，损坏 armed pending 返回 failure 并保留当前 store。`CanonicalMigrationSafetyGate` 要求 destructive migration / cutover 调用方先创建并立即校验 `.schemaMigration` 恢复点，创建或校验失败必须抛错中止。当前还没有设置页 canonical 恢复点生产 UI、普通 SwiftUI 生产启动接入或真实 cutover 调用方。 | `CanonicalStore.swift`、`CanonicalRecords.swift`、`CanonicalRecoveryPointStore.swift`、`CanonicalRecoveryPointSnapshotService.swift`、`CanonicalRecoveryCoordinator.swift`、`CanonicalRestoreExecutor.swift`、`CanonicalBootRestoreGate.swift`、`CanonicalMigrationSafetyGate.swift`、`CanonicalLibraryRuntime.swift` |
...
结果：通过。`MarkdownExportServiceTests` 覆盖从 SwiftData 活跃 Moment 快照生成 Markdown、写入相对 `assets/` 附件、保留照片字节、排除软删除记录，以及写入失败后清理未完成导出包；`MarkdownExportRendererTests` 覆盖标题/标签 Markdown 转义、正文换行归一和图片扩展名识别；`MarkdownExportUITests` 覆盖设置页“导出”入口、详情页说明、生成 Markdown、成功态和系统分享入口。分享入口指向整个导出目录，避免只分享 `.md` 时丢失相对 `assets/`。当前 M1 只实现全部活跃时刻导出；筛选/日期范围、PDF、取消/重试和持久 `export_job` 仍在计划层。
