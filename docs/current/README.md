...
| 数据持久化 | M3 后，生产主 UI、用户写入和本机恢复点路径的权威均为 GRDB canonical store。`MoodmentsApp` 启动先调用 `CanonicalBootRestoreGate` 消费 pending restore，再创建并注入 `CanonicalLibraryService` / `CanonicalLibraryRuntime` / `CanonicalRecoveryCoordinator`；`RootView` 在进入主页前执行一次 SwiftData -> canonical baseline 导入，若本次启动刚完成 canonical restore 则跳过 baseline 导入。时间轴、筛选、预览、编辑、标签管理、垃圾箱、统计/热力图、缩略图原图读取和主流程写入已消费 canonical 值类型与 `changeToken`。App 仍创建 SwiftData `ModelContainer`，用于 baseline 导入、M4 前的过渡导出和部分测试种子；同步状态仍是启发式展示，不代表 canonical iCloud 同步已经完成。 | `ModelContainer+Config.swift`、`MoodmentsApp.swift`、`RootView.swift`、`CanonicalLibraryService.swift`、`LocalLibraryMutationService.swift`、`SyncStatusService.swift`、`CanonicalStore.swift`、`CanonicalLibraryRuntime.swift`、`FileAssetStore.swift`、`SwiftDataCanonicalImporter.swift`、`CanonicalLibraryRepository.swift` |
