...
| Markdown / PDF 导出 | M4 canonical export cutover 已落地，M5 已删除 SwiftData 导出 snapshot adapter。设置页“导出”详情页可选择 Markdown 或 PDF，生产 `ExportView` 从环境中的 `CanonicalLibraryService.repository` 构造 `CanonicalExportSnapshotStore`，读取 canonical 活跃 Moment，按 `occurredAt` 倒序生成导出输入；标签按 canonical link 顺序输出，图片按 `moment_asset_link.sort_index` 从 `FileAssetStore` 读取原图 bytes。Markdown 使用 `MarkdownExportRenderer` 生成 `.md` 和相邻 `assets/` 相对图片目录，PDF 使用系统 `UIGraphicsPDFRenderer` / CoreText 本地分页并把照片嵌入 PDF。导出仍是只读副本，不改变 canonical store，不创建恢复点，不参与 iCloud 同步；当前只支持全部活跃时刻，不支持筛选/日期范围/持久 `export_job`。 | `ExportService.swift`、`MarkdownExportRenderer.swift`、`PDFExportRenderer.swift`、`ExportView.swift` |
