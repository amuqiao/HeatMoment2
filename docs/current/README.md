...
| Markdown / PDF 导出 | 已落地 M1。设置页“导出”详情页可选择 Markdown 或 PDF，从当前 SwiftData 活跃 Moment 快照生成只读副本；渲染和文件写入在 UI actor 外执行。Markdown 照片复制到相邻 `assets/` 并以相对链接引用，分享整个导出目录；PDF 使用系统 renderer/CoreText 分页并嵌入照片，分享单个 `.pdf` 文件。导出包位于系统临时目录，每次新导出前清理同格式旧包。当前只支持全部活跃时刻，不支持筛选/日期范围/持久导出任务。 | `ExportService.swift`、`MarkdownExportRenderer.swift`、`PDFExportRenderer.swift`、`ExportView.swift` |
