...
| 数据持久化 | M4 后，生产主 UI、用户写入、本机恢复点和 Markdown/PDF 导出路径的权威均为 GRDB canonical store。`MoodmentsApp` 启动先调用 `CanonicalBootRestoreGate` 消费 pending restore，再创建并注入 `CanonicalLibraryService` / `CanonicalLibraryRuntime` / `CanonicalRecoveryCoordinator`；`RootView` 在进入主页前执行一次 SwiftData -> canonical baseline 导入，若本次启动刚完成 canonical restore 则跳过 baseline 导入。时间轴、筛选、预览、编辑、标签管理、垃圾箱、统计/热力图、缩略图原图读取、主流程写入和导出 snapshot 已消费 canonical 值类型与 `FileAssetStore` 原图。App 仍创建 SwiftData `ModelContainer`，用于 baseline 导入和部分测试种子；同步状态仍是启发式展示，不代表 canonical iCloud 同步已经完成。 | `ModelContainer+Config.swift`、`MoodmentsApp.swift`、`RootView.swift`、`CanonicalLibraryService.swift`、`LocalLibraryMutationService.swift`、`SyncStatusService.swift`、`CanonicalStore.swift`、`CanonicalLibraryRuntime.swift`、`FileAssetStore.swift`、`SwiftDataCanonicalImporter.swift`、`CanonicalLibraryRepository.swift`、`ExportService.swift` |
...
| Markdown / PDF 导出 | M4 canonical export cutover 已落地。设置页“导出”详情页可选择 Markdown 或 PDF，生产 `ExportView` 从环境中的 `CanonicalLibraryService.repository` 构造 `CanonicalExportSnapshotStore`，读取 canonical 活跃 Moment，按 `occurredAt` 倒序生成导出输入；标签按 canonical link 顺序输出，图片按 `moment_asset_link.sort_index` 从 `FileAssetStore` 读取原图 bytes。Markdown 使用 `MarkdownExportRenderer` 生成 `.md` 和相邻 `assets/` 相对图片目录，PDF 使用系统 `UIGraphicsPDFRenderer` / CoreText 本地分页并把照片嵌入 PDF。导出仍是只读副本，不改变 canonical store，不创建恢复点，不参与 iCloud 同步；当前只支持全部活跃时刻，不支持筛选/日期范围/持久 `export_job`。SwiftData 导出 snapshot store 仅作为过渡代码保留，不再是生产导出 wiring。 | `ExportService.swift`、`MarkdownExportRenderer.swift`、`PDFExportRenderer.swift`、`ExportView.swift` |
