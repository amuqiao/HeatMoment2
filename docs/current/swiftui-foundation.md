...
- 备份恢复：`BackupRestoreServicing` 由设置页消费，`CanonicalBackupRestoreService` 由 App composition 注入。
- 导出：`ExportServicing` 由导出详情页消费，`CanonicalExportService` 负责接入 canonical snapshot adapter。
