...
设置页根页当前提供“备份/还原”入口，push 到 `BackupRestoreView`；该详情页默认表达完整备份能力，消费 `BackupPackageServicing`，生产由 App composition 注入 `CanonicalBackupPackageService`。页面默认只显示当前内容、上次备份、完整备份操作选择和单个主操作按钮；导出备份与导入还原互斥选择，不再作为并列主按钮展示，本机 recovery point、schema version 和来源版本不作为常驻用户概念展示。导出会先在 `BackupPackageExports/Prepared/<packageID>/` 生成临时 `.heatmomentbackup` 完整包，并立即打开系统分享/保存；只有 `UIActivityViewController` 返回 completed 后，service 才记录上次备份时间，并用 `.share-completed` marker 保留本次分享源包一小时，避免 Files / iCloud Drive 在保存收尾时遇到已删除的源文件。用户取消分享或重新进入页面时，未完成的临时包会被清理；已完成的临时包超过保留窗口后由显式清理入口清理。App 不承诺知道用户后续保存路径，不替换或删除外部旧备份。导入使用系统 file importer，先复制文件到 App staging，再校验 magic、manifest length、manifest、payload 相对路径、字节数、SHA-256、尾部数据、SQLite catalog 和 asset manifest；校验通过后展示备份时间、内容计数和确认替换入口，用户确认后才写入资产、创建还原前安全点、stage pending restore 并 arm 下次冷启动替换。
