...
`Sources/Moodments/Persistence/Canonical/` 是 GRDB canonical local core。它当前包含 SQLite migration、library metadata、moment/tag records、稳定排序的 moment-tag link、asset/link metadata、content-addressed `FileAssetStore`、asset reachability audit、GC dry-run plan、DB orphan asset record finalizer、orphan blob cleanup、tombstone、mutation log、`CanonicalLibraryRepository` 的事务边界、`CanonicalLibraryRuntime` 装配类型，以及 SwiftData -> canonical baseline 导入器。当前 `MoodmentsApp` 不在普通启动时打开 canonical 持久库，因为 SwiftData 过渡版本地恢复点只替换 `Moodments.store*`，还不管理 `Application Support/Canonical/`；在受控 cutover 之前提前物化第二个持久库会制造恢复边界外的 stale canonical 风险。导入器保留 SwiftData 的业务 ID、时间戳、软删除状态、稳定标签顺序、图片顺序和 asset metadata，并把 SwiftData `MomentImage.imageData` 原图字节写入 content-addressed asset store；相同 bytes 共享同一个 blob，SQLite 仍用 `asset_record.id` / `moment_asset_link` 保留业务图片身份和顺序。baseline 导入不写 `mutation_log` 或 `tombstone_record`；导入完成会记录 source fingerprint，重复导入只有同一 source fingerprint 才跳过；源数据异常、目标 canonical 已有业务表或历史表内容、source fingerprint 不一致时都会失败并回滚，不做 upsert 合并。

现阶段 canonical runtime/importer 只作为后续 SwiftData -> canonical 受控 cutover、恢复点 catalog、导出和 iCloud sync follow-up 的本地权威地基；它尚未接入 SwiftUI 用户读写路径，也不和 SwiftData 双写。生产用户仍实际使用 SwiftData 路径。`CanonicalAssetReachabilityService` 当前只做维护面审计和受保护 cleanup：从 `asset_record`、`moment_asset_link` 和 asset store 文件系统聚合 `content_hash`，报告无 link 的 asset record、negative `pin_count`、missing blob、corrupt blob、invalid hash/path，以及 DB 无引用的 orphan blob。`planGarbageCollection()` 是 dry-run：列出可 finalize 的 unlinked + unpinned `asset_record`、当前 orphan blob，以及 finalize 后才会变成可删的 blob hash。`finalizeUnlinkedAssetRecords()` 在 `CanonicalAssetOperationGate` 内执行，并在 DB write transaction 中重新确认整批 planned record 仍然 no-link 且 `pin_count == 0` 后只删除 `asset_record`；若整批条件不一致则抛出 race 错误且不提交部分删除。它不删除 blob。`cleanupOrphanBlobs()` 只在没有 blocking issue 时删除 DB 无引用 orphan blob，不参与业务写入事务。`CanonicalAssetOperationGate` 是按标准化 asset root 路径共享的本进程互斥门闩：SwiftData baseline import 的“blob 落盘 -> SQLite metadata/link 写入”、record finalizer 和 orphan cleanup 对同一 asset root 复用同一个 gate，避免 cleanup 删除尚未提交到 DB 的合法 blob。后续 canonical UI 写入原图时也必须通过同一 asset root gate；如果出现跨进程后台任务，则需升级为持久 staging/pin 方案。当前还没有独立 asset pin 表、恢复点 asset manifest、restore staging pin、export job pin 或 sync pin；现有 `asset_record.pin_count` 只能作为按 `content_hash` 聚合后的保护信号，不是最终 GC 依据。定向测试覆盖 schema migration、Moment 生命周期、`purgePending`、标签 create-or-reuse、额度、rename/delete、删除标签时受影响 Moment 的 revision/mutation、tag tombstone、缺失引用回滚、稳定排序约束、重名冲突回滚、runtime 落盘建库、SwiftData baseline 导入、导入完成标记与 source fingerprint、重复导入跳过、source mismatch 拒绝、异常源数据回滚、非空目标或仅有历史表内容时拒绝导入、baseline 不污染 mutation log、content-addressed 原图写入/读回/去重、baseline 导入后的 asset hash 与文件字节一致，以及 asset reachability audit / GC dry-run / DB orphan record finalizer / orphan blob cleanup 的安全边界。
